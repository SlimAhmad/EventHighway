...
namespace EventHighway.Core.Tests.Unit.Services.Orchestrations.RestoringEvents.V2
{
    ...
    {
        [Fact]
        public async Task ShouldGenerateReplayListenerEventV2sForSpecifiedListenersOnlyAsync()
        {
            // given
            ...
            EventListenerV2 targetEventListenerV2 = CreateRandomEventListenerV2s().First();
            EventListenerV2 otherEventListenerV2 = CreateRandomEventListenerV2s().First();

            List<Guid> inputEventListenerIds =
                new List<Guid> { targetEventListenerV2.Id };

            IQueryable<EventListenerV2> eventListenerV2s =
                new List<EventListenerV2> { targetEventListenerV2, otherEventListenerV2 }
                    .AsQueryable();

            IQueryable<ListenerEventV2> existingListenerEventV2s =
                new List<ListenerEventV2>().AsQueryable();
            ...
            // when
            await this.restoringEventV2OrchestrationService.GenerateReplayForListenersAsync(
