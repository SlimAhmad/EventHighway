...
namespace EventHighway.Core.Tests.Unit.Services.Foundations.Events.V2
{
    ...
    {
        [Fact]
        public async Task ShouldRetrieveAllEventV2sWithListenerEventV2sAsync()
        {
            // given
            ...
            this.storageBrokerMock.Setup(broker =>
                broker.SelectAllEventV2sWithListenerEventV2sAsync(randomCancellationToken))
                    .ReturnsAsync(retrievedEventV2s);

            // when
            IQueryable<EventV2> actualEventV2s =
                await this.eventV2Service
                    .RetrieveAllEventV2sWithListenerEventV2sAsync(randomCancellationToken);
